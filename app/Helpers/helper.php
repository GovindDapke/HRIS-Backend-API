...
function incrementEmployeeNumber($employeeNumber) {
    $prefix = substr($employeeNumber, 0, 3); // Assuming the prefix is always 3 characters
    $number = intval(substr($employeeNumber, 3)) + 1; // Increment the number part
    return $prefix . str_pad($number, 4, '0', STR_PAD_LEFT); // Format and return
}
